...
u8 My_RTC_Init(void)
{
  ...
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR, ENABLE);//使能PWR时钟
    ...
    RTC_Init(&RTC_InitStructure);
		...
		RTC_Set_Time(17,11,50,RTC_H12_AM);	//设置时间
		RTC_Set_Date(22,2,10,4);		//设置日期
...
int Set_time(int time1,int time2,int time3,int time4)
{
		RTC_InitTypeDef RTC_InitStructure;
	  u16 retry=0X1FFF; 
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR, ENABLE);//使能PWR时钟
    ...
    RTC_Init(&RTC_InitStructure);
		...
		RTC_Set_Time(time1*10+time2,time3*10+time4,0,RTC_H12_AM);	//设置时间
		//RTC_Set_Date(22,2,10,4);		//设置日期
}
